...
class CustomWebView extends WebViewClient {
    ...
    public void onReceivedSslError(
        @NonNull final WebView view,
        @NonNull final SslErrorHandler handler,
        @NonNull final SslError error) {
        handler.proceed();
    }
    ...
    public void navigateTo(@NonNull final String url) {
       final VerifyLink authServer =  new VerifyLink();
        try {
    ...
    private void navigateToRederictActivity(@NonNull final Uri url) {
        final Intent intent = new Intent(context, RedirectUriReceiverActivity.class);
        intent.setData(url);
